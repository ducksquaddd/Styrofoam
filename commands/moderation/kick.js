...
const data = new SlashCommandBuilder()
    .setName("kick")
    .setDescription("Kicks a person on your Discord server.")
    .addMentionableOption(option =>
        option.setName('target').setDescription('Select a user.').setRequired(true)
    )
    .addStringOption(option =>
        option.setName('reason').setDescription('Reason as to why you want to kick this user.').setRequired(true)
    )
        ...
        if(member.moderatable) {
            try {
                member.kick(reason).then((x) => {
                    i.reply(`Kicked ${x.user.username}. Reason: ${reason}`)
                });
            } catch (err) {
                console.log(err);
            }
